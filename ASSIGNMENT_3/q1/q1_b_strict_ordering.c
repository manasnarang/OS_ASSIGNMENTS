...
//Strict order of resource requests
...
void lock_bowl(int bowl_num,int philosopher){
	while(bowls[bowl_num]!=-1);		//waiting for lock to be released
	bowls[bowl_num]=philosopher;
    phil_bowls[philosopher]=bowl_num;
	printf("Philosopher %d has picked up bowl %d\n",philosopher,bowl_num);
}

void release_bowl(int bowl_num, int philosopher){
...
void get_bowl(int philosopher){
    if(bowls[0]==-1){
        lock_bowl(0,philosopher);
    }
    else{
        lock_bowl(1,philosopher);
    }
}

void put_bowl(int philosopher){
    release_bowl(phil_bowls[philosopher],philosopher);
}
...
int main(){
	...
	//initialise the forks and phil_bowls
	for(int i=0;i<5;i++){
